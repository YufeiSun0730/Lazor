...
class Lasor():
    ...
    def collide_A(collision_dir):
        if collision_dir == "vertical":
            direction_out = (self.direction[0], -self.direction[1])
        if collision_dir == "horizontal":
            direction_out = (-self.direction[0], self.direction[1])
        return direction_out

    def collide_B(collision_dir):
        direction_out = None
        return direction_out

    def collide_C(collision_dir):
        if collision_dir == "vertical":
    ...
    def current_lazor_path(start, direction, end):
        ...
        while current_pos != final_pos:
            ...
            def check_grid_type(position, direction):
                if position[0] // 2 == 1:
                    if direction[1] > 0:
            ...
            grid_type = check_grid_type(next_pos, current_dir)[0]
            collision_dir = check_grid_type(next_pos, current_dir)[1]
            if grid_type == "A":
                next_dir = collide_A(collision_dir)
            elif grid_type == "B":
                next_dir = collide_B(collision_dir)
            elif grid_type == "C":
                next_dir = collide_C(collision_dir)
            else:
...
def pos_check(coord, grid):
    if coord[0] >= 0 and coord[0] <= len(grid) and coord[1] >= 0 and coord[1] <= len(grid):
        return True
    else
        return False

def find_block_coord(vector, initial_coord):
    block_coord_list = []
    if vector[0] < 0 and vector[1] < 0:
        if board[coord[0]][coord[1]].findentdir() == 'side':
            block_coord = [initial_coord[0] + vector[0] - 1, initial_coord[1] + vector[1]]
        if board[coord[0]][coord[1]].findentdir() == 'vert':
            block_coord = [initial_coord[0] + vector[0] , initial_coord[1] + vector[1] + 1]
        block_coord_list = block_coord_list.append(block_coord)
