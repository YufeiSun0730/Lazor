...
class Block(object):
    ...
    def pass_through(self, in_position: Tuple[int, int],
                     direction: Tuple[int, int]) -> List['Lazor']:
        '''
...
class EmptyBlock(Block):   # class block X
    def pass_through(self, in_position: Tuple[int, int],
        ...
        **functions**
        ...
        This function takes in two tuple arguments and will

        return return new lazor object
        ...
        **Parameters**
        ...
        new_lazor = Lazor(grid=self.grid, start=in_position,
                          direction=direction)
...
class ReflectBlock(Block):  # class block A
    def pass_through(self, in_position: Tuple[int, int],
        ...
        **Parameters**
        ...
        if reflect_direction == 0:  # reflect on the side
            new_direction = (- direction[0], direction[1])
        else:  # redlect on top and bot
            new_direction = (direction[0], - direction[1])

        return [Lazor(grid=self.grid, start=in_position,
    ...
    **class**
    ...
    def pass_through(self, in_position: Tuple[int, int],
        ...
        **functions**
        ...
        return new lazor object (which is none)
...
class RefractBlock(Block):  # class block C
    ...
    def pass_through(self, in_position: Tuple[int, int],
        ...
        **functions**
        ...
        This function takes in two tuple arguments and

        will return two lazor objects
        ...
        **Parameters**
        ...
        '''
        to_return = []
        to_return.extend(self.empty_block.pass_through
                         (in_position, direction))
        # laser paththrough
        to_return.extend(self.reflect_block.pass_through
                         (in_position, direction))
        # laser also reflect
        return to_return
...
class Grid(object):
    ...
    **Returns**
    ...
    def is_block_available(self, central: Tuple[int, int]) -> bool:
        '''
        This function is to check the center coordinates for available
    ...
    def within_grid(self, position: Tuple[int, int]) -> bool:
        ...
        return 0 <= position[0] < self.x_limit and \
            0 <= position[1] < self.y_limit
    ...
    def get_laser_in_block(self,
        ...
        start_point_centrals = self.nearby_centrals
        (in_position[0], in_position[1])
        end_point_centrals = self.nearby_centrals
        (in_position[0] + direction[0],
    ...
    def read_bff(cls, filename: str) -> 'Game':
        ...
        **Returns**
        ...
        with open(filename, 'r') as f:
            ...
            for line in f:
                ...
                if line.startswith('GRID START'):
                    ...
                    line_splits = line.split()
                    intersection_points.append((int(line_splits[1]),
                                                int(line_splits[2])))
        ...
        return cls(grid=init_grid, intersection_points=intersection_points,
                   ...
                   init_lazor=Lazor(grid=init_grid, start=init_lazor[0],
                                    direction=init_lazor[1]),)
...
def build_lazor_path(grid: Grid,
                     ...
                     is_init: bool = False):
    '''
    This function takes in a grid obj, a lazer_path obj, a Lazor obj, 

    a list, and a bool, and returns function.
    '''
    if not is_init:
    ...
    laser_in_block: Block = grid.get_laser_in_block(
        in_position=block_in_pos, direction=current_lazor.direction)
    blocks_along_path.append(laser_in_block.central)
    new_lazors = laser_in_block.pass_through(
        in_position=block_in_pos, direction=current_lazor.direction)
    for new_lazor in new_lazors:
        build_lazor_path(grid=grid, current_lazor=new_lazor,
                         lazor_path=lazor_path,
                         blocks_along_path=blocks_along_path,
                         is_init=False)
...
def solve_game(game: Game) -> Optional[Grid]:
    ...
    def _solve() -> bool:  # check empty value
        ...
        build_lazor_path(grid=game.grid, lazor_path=current_lazor_path,
                         ...
                         blocks_along_path=current_available_blocks,
                         is_init=True)
        ...
        for block in current_available_blocks:
            ...
            remaining_actions: List[str] = ['o'] + [
                k for k, v in game.blocks_to_place.items() if v > 0]
            for action in remaining_actions:
                # need to figure out the action type
                if action == 'o':
                    place_success = True
                elif action == 'A':
                    new_block = ReflectBlock(central=block, grid=game.grid)
                    place_success = game.grid.place_block(
                        central=block, block=new_block)
                elif action == 'B':
...
if __name__ == '__main__':
    game = Game.read_bff(
        filename='/Users/mordredyuan/Downloads/Lazor-main/Lazor\
        ProjectFall2021/mad_4.bff')
